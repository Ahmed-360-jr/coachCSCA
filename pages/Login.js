...
export default function Login({ store }) {
    ...
    try {
        ...
        const resultsRes = await fetch(`${API_BASE}/api/mock/my-results`, {
            headers: {
        ...
        if (resultsRes.ok) {
            store.setMockHistory(resultsData);
        } else {
    ...
    return html`
        <div class="relative min-h-screen flex items-center justify-center px-4 py-16 overflow-hidden">
            <div class="absolute inset-0 pointer-events-none">
                <div class="absolute top-16 right-10 w-72 h-72 bg-primary/10 blur-3xl rounded-full"></div>
                <div class="absolute bottom-10 left-10 w-80 h-80 bg-secondary/10 blur-3xl rounded-full"></div>
                ...
                <div class="glass-card p-8 md:p-10 rounded-[2rem] shadow-2xl border border-white/20 dark:border-slate-700/50 backdrop-blur-xl">
                    <div class="text-center mb-8">
                        ...
                        <p class="text-slate-500 dark:text-slate-300 text-sm md:text-base max-w-sm mx-auto">
                            Continue your preparation journey and get back to focused practice and mock tests.
                        </p>
                    </div>
                    ...
                    <p class="mt-6 text-center text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
                        Access your account and continue building your confidence for the CSCA exam.
                    </p>
